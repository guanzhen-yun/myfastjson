...
public class TypeUtils {
    ...
    private static List<FieldInfo> computeSetters(Class<?> clazz, Map<String, Field> fieldCacheMap) {
        ...
        for(Method method : methods) {
            ...
            if(methodName.startsWith("set")) {//有些属性没有定义全局，具有get或者is方法
                //方法名至少为: setX(),不能为set()
                ...
                //将setXyz()转为：xyz属性名 （索引012为set,索引3大写转小写，截取到最后）
            ...
            if (field != null) {// 省略了一些条件筛选，做健壮判断
                //新增属性 getter/setter 方法(参考源码: TypeUtils.java 1927/2000行)
        ...
        //同样有些属性是public定义的，但是没提供getter/setter方法(参考源码: TypeUtils.java 2005行)
        ...
        for (Field field : fields) {
            ...
            //加入集合，最后返回整个集合的value，也就是序列化对象的所有属性信息
            fieldInfoMap.put(propertyName, fieldInfo);
        }
    ...
    /**
     * 计算 /收集getter 方法(参考源码TypeUtils.java 1722行)
    ...
    private static List<FieldInfo> computeGetters(Class<?> clazz, Map<String, Field> fieldCacheMap) {
        ...
        for(Method method : methods) {
            ...
            //参考源码TypeUtils.java 1850行
            if(methodName.startsWith("get")) {//有些属性没有定义全局，具有get或者is方法
                //方法名至少为: getX(),不能为get()
                if(methodName.length()<4) {
                    continue;
                }
            ...
            //boolean方法的get是is开头(pricate boolean isHero;) (参考源码TypeUtils.java 1931行)
            if(methodName.startsWith("is")) {//有些属性没有定义全局，只有get或者is方法
                ...
                //优先选中get, 源码TypeUtils.java 1996行(修改版)
                if(fieldInfoMap.containsKey(temp)) {
                    //集合中有，说明get方法已经存储了
                    continue;
                }
    ...
    private static void parserAllFieldToCache(Class<?> clazz, Map<String, Field> fieldCacheMap) {
        ...
        //递归寻找当前序列化对象的父类，直到Object为止 (如UserInfo extends BaseUser)
